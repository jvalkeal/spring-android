...
public class TcpNioClientConnectionFactory extends
    ...
    private final static String TAG = "TcpNioClientConnectionFactory";
	...
	protected TcpConnection obtainConnection() throws Exception {
		...
		if (LogConstants.DEBUG) {
			Log.d(TAG, "Opening new socket channel connection to " + this.getHost() + ":" + this.getPort());
		}
	...
	public void run() {
        if (LogConstants.DEBUG) {
            Log.d(TAG, "Read selector running for connections to " + this.getHost() + ":" + this.getPort());
        }
		try {
			...
			while (this.isActive()) {
				...
				try {
			        ...
			        if (LogConstants.DEBUG) {
			            Log.d(TAG, "CancelledKeyException during Selector.select()");
			        }
				}
				while ((newChannel = newChannels.poll()) != null) {
					try {
	                    ...
	                    if (LogConstants.DEBUG) {
	                        Log.d(TAG, "Channel closed before registering with selector for reading");
	                    }
					}
				}
			...
			Log.e(TAG, "Exception in read selector thread", e);
			this.setActive(false);
		}
        if (LogConstants.DEBUG) {
            Log.d(TAG, "Read selector exiting for connections to " + this.getHost() + ":" + this.getPort());
        }
	}

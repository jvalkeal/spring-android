...
import org.springframework.LogConstants;
...
public abstract class ConcurrencyThrottleSupport implements Serializable {
    ...
    private static final String TAG = "ConcurrencyThrottleSupport";
			...
			synchronized (this.monitor) {
				...
				while (this.concurrencyCount >= this.concurrencyLimit) {
					...
					if(LogConstants.DEBUG) {
						Log.d(TAG, "Concurrency count " + this.concurrencyCount +
				...
				if(LogConstants.DEBUG) {
				    Log.d(TAG, "Entering throttle at concurrency count " + this.concurrencyCount);
				}
	...
	protected void afterAccess() {
		if (this.concurrencyLimit >= 0) {
			synchronized (this.monitor) {
				this.concurrencyCount--;
				if(LogConstants.DEBUG) {
				    Log.d(TAG, "Returning from throttle at concurrency count " + this.concurrencyCount);
				}
				this.monitor.notify();
			}
		}

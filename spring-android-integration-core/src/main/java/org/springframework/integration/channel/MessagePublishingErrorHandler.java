...
//	private final Log logger = LogFactory.getLog(this.getClass());
    private final static String TAG = "MessagePublishingErrorHandler";
		...
		if (errorChannel != null) {
			try {
				...
				if (LogConstants.DEBUG) {
					Log.w(TAG, "Error message was not delivered.", errorDeliveryError);
				}
			}
		}
		if (!sent && LogConstants.DEBUG) {
			Message<?> failedMessage = (t instanceof MessagingException) ?
					((MessagingException) t).getFailedMessage() : null;
			if (failedMessage != null) {
				Log.e(TAG, "failure occurred in messaging task with message: " + failedMessage, t);
			}
			else {
			    Log.e(TAG, "failure occurred in messaging task", t);
			}
		}
	}

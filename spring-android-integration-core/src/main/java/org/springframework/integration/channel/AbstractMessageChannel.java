...
public abstract class AbstractMessageChannel /*extends IntegrationObjectSupport*/ implements MessageChannel/*, TrackableComponent*/ {
    ...
    private final static String TAG = "AbstractMessageChannel";
...
//			}
//		}
		throw new MessageDeliveryException(message, "Channel expected one of the following datataypes [" + 
	...
	protected class ChannelInterceptorList {
		...
		public Message<?> preSend(Message<?> message, MessageChannel channel) {
			if (LogConstants.DEBUG) {
				Log.d(TAG, "preSend on channel '" + channel + "', message: " + message);
			}
		...
		public void postSend(Message<?> message, MessageChannel channel, boolean sent) {
			if (LogConstants.DEBUG) {
				Log.d(TAG, "postSend (sent=" + sent + ") on channel '" + channel + "', message: " + message);
			}
			for (ChannelInterceptor interceptor : interceptors) {
		...
		public boolean preReceive(MessageChannel channel) {
			if (LogConstants.VERBOSE) {
			    Log.v(TAG, "preReceive on channel '" + channel + "'");
			}
			for (ChannelInterceptor interceptor : interceptors) {
		...
		public Message<?> postReceive(Message<?> message, MessageChannel channel) {
			if (message != null && LogConstants.DEBUG) {
				Log.d(TAG, "postReceive on channel '" + channel + "', message: " + message);
			}
			else if (LogConstants.VERBOSE) {
			    Log.v(TAG, "postReceive on channel '" + channel + "', message is null");
			}
			for (ChannelInterceptor interceptor : interceptors) {
